...
func NewRouter(st *store.Store, cfg *config.Config) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/applications/{id}/ip-overrides", handlers.ListAppIPOverrides(st))
	mux.HandleFunc("POST /api/v1/applications/{id}/ip-overrides", handlers.AddAppIPOverride(st))
	mux.HandleFunc("DELETE /api/v1/applications/{id}/ip-overrides/{entryId}", handlers.DeleteAppIPOverride(st))
	...
	mux.HandleFunc("POST /api/v1/managed-rules/global", handlers.ManageCRSGlobally(st))
	mux.HandleFunc("GET /api/v1/rules/managed", handlers.ListManagedRules(st))
	mux.HandleFunc("PATCH /api/v1/rules/managed/{id}", handlers.ConfigureManagedRules(st))
	mux.HandleFunc("POST /api/v1/rules/managed/global", handlers.ManageCRSGlobally(st))
	...
	mux.HandleFunc("GET /api/v1/reports", handlers.ListReports(st))
	mux.HandleFunc("POST /api/v1/reports", handlers.CreateReport(st))
	mux.HandleFunc("GET /api/v1/reports/{id}", handlers.GetReport(st))
	mux.HandleFunc("DELETE /api/v1/reports/{id}", handlers.DeleteReport(st))
	mux.HandleFunc("POST /api/v1/reports/security", handlers.GenerateSecurityReport(st))
	mux.HandleFunc("POST /api/v1/reports/traffic", handlers.GenerateTrafficReport(st))
	mux.HandleFunc("POST /api/v1/reports/incidents", handlers.GenerateIncidentsReport(st))
	mux.HandleFunc("POST /api/v1/reports/compliance", handlers.GenerateComplianceReport(st))
	mux.HandleFunc("GET /api/v1/reports/{id}/download", handlers.DownloadReport(st))
